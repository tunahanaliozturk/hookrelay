...
// the .nupkg, so a dependency can carry a commercial subscription agreement or a maintenance-fee demand
// and nothing in the build will mention it: the package restores, the code compiles, and that is that.
// Both happened in this portfolio, which is why this runs on every build.
//
...
Console.WriteLine();
Console.WriteLine("Replace the dependency, or add its licence to the allow list in tools/HookRelay.LicenseAudit.");
return 1;
